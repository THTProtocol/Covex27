...
pub fn open_db(path: &str) -> anyhow::Result<Db> {
    ...
    conn.execute_batch(
        ...
        -- The first-party privacy-mixer tables (mixer_leaves / mixer_roots / mixer_nullifiers)
        -- were removed with the privacy-mixer co-sign path (legal / sanctions posture). No code
        -- reads or writes them. Any pre-existing tables in a deployed DB are simply left unused.
