...
pub async fn run_payment_verifier(
    ...
    info!("Payment Verifier v2 started -- monitoring treasury: {}", treasury_address);
    ...
    loop {
        ...
        let current_daa = match client.get_block_dag_info().await {
            ...
            Err(e) => { warn!("Payment Verifier: get_block_dag_info failed: {}", e); continue; }
        };

        let addr = match kaspa_addresses::Address::try_from(treasury_address.as_str()) {
            Ok(a) => a,
            Err(e) => { error!("Payment Verifier: invalid treasury address: {}", e); continue; }
        };
            ...
            Ok(entries) => {
                for entry in entries {
                    ...
                    if let Some(tier) = tier_from_amount(amount_sompi) {
                        let confirmations = if current_daa > block_daa { current_daa - block_daa } else { 0 };

                        // MATCH payment to covenant by from_address == creator_addr
                        let matched_covenant = match db::get_covenants_by_creator(&db, &from_address) {
                            Ok(list) => list.into_iter().next(),
                            Err(_) => None,
                        };

                        let covenant_id: Option<String> = matched_covenant.as_ref().map(|c| c.tx_id.clone());

                        if let Err(e) = db::insert_payment(&db, &tx_id, &from_address, &treasury_address, amount_sompi, tier, covenant_id.as_deref()) {
                            warn!("Payment Verifier: insert_payment failed for {}: {}", tx_id, e);
                            continue;
                        }

                        if confirmations >= 6 {
                            if let Err(e) = db::confirm_payment(&db, &tx_id, confirmations as i64) {
                                warn!("Payment Verifier: confirm_payment failed for {}: {}", tx_id, e);
                            } else {
                                // Upgrade the account
                                if let Err(e) = db::upgrade_account(&db, &from_address, tier, &tx_id) {
                                    error!("Payment Verifier: upgrade_account failed for {}: {}", from_address, e);
                                } else {
                                    ...
                                    if let Some(ref cid) = covenant_id {
                                        ...
                                        let addresses = matched_covenant.as_ref().map(|c| c.address.clone()).unwrap_or_default();

                                        if let Err(e) = db::upgrade_covenant_record(&db, cid, tier, &tx_id, &full_summary, &addresses) {
                                            ...
                                            let gen_name = format!("Verified {}", matched_covenant.as_ref().map(|c| c.covenant_type.as_str()).unwrap_or("Covenant"));
                                            let gen_cat = matched_covenant.as_ref().map(|c| c.category.clone()).unwrap_or_else(|| "General".into());
                                            let gen_hash = matched_covenant.as_ref().map(|c| c.script_hash.clone()).unwrap_or_default();
                            ...
                            debug!("Payment Verifier: Pending payment {} ({} confs, needs 6)", &tx_id[..16], confirmations);
                        }
                    }
                }
            }
            Err(e) => { warn!("Payment Verifier: get_utxos_by_addresses failed: {}", e); }
        }
    }
}
