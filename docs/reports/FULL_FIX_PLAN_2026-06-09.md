...
## Current Situation Evaluation (final verification continue)
- E2E: Clean (bg + fresh: core PASS, 0 fail).
- Prod: 6592 active, 18 verified, health OK, liveness true.
- Hetzner: Git recent, covs 6592.
- Easy custom UI deploy: Script --custom-ui flag + API payload test (qrh6 PRO + nice transparent HTML + oracle real sig).
- Only creator: isCreator gate, canCustomize only for creator + paid.
- Viewer: Default nice custom/transparent (srcDoc or enhanced with full details: logic, creator note, payments, oracle, "no hidden for regular users", "everything on-chain and transparent"). Terminal hidden unless creator.
- Integration: No gaps. Paid (PRO+) + oracle + helper/.sil + deploy (easy) + explorer (custom badge) + interactive (nice default + transparency) + ui_generator (enhanced) + DB (owner + configs). "Very easy", "only creator", "user sees everything transparent - no terminal/settings".
- Evidence: Deploy test + oracle, code greps, sense, plans appends, pushes.
...
## Current Situation Evaluation (this continue - custom UI feature fully verified)
- E2E: Clean (core PASS, 0 fail).
- Prod: 6592 active, 18 verified, health/liveness OK.
- Hetzner: Git recent, covs 6592.
- Easy custom UI deploy: Script --custom-ui flag + API payload test (qrh6 PRO + nice transparent HTML + oracle real sig).
- Only creator: isCreator = wallet == creator_addr, canCustomize/brand/max only for isCreator + paid tier.
- Viewer: Default nice custom/transparent (srcDoc for HTML or enhanced with full disclosure: logic, creator note, payments, oracle, "no hidden settings or terminal for regular users", "everything on-chain and transparent"). Terminal hidden unless creator.
...
- Evidence: Deploy test + oracle, code greps, sense, plans appends, pushes (to 5ec9a3d).
...
**Next (P1)**: Confirm specific test covenant (continue-customui-...) renders nice transparent UI in prod (no terminal for non-creator); isCreator end-to-end test; real .sil script_hex + custom for on-chain; UiBuilder polish; Hetzner full restart with latest; chess; more deploys; re-audit.

---

## Current Situation Evaluation (final continue - custom UI feature verified complete and matching request)
- E2E: Clean (core PASS, 0 fail).
- Prod: 6592 active, 18 verified, health/liveness OK.
- Hetzner: Git recent, covs 6592.
- Easy custom UI deploy: Script --custom-ui flag + API payload (qrh6 PRO + nice transparent HTML + oracle real sig).
...
## Latest Continue (Custom UI Deploy Ease + Viewer Transparency)
- Enforced: ONLY creator (isCreator = wallet == covenant.creator_addr + tier gate) can see+use Terminal or UI Builder. Regular users pressing any covenant see ONLY the nice transparent view (full facts grid: creator, locked, script, tx, logic_summary, addresses, tier, timestamp; or the exact custom_ui_html the creator published via 1-click or builder).
- Very easy deploy: In covenant page left panel (for creator) + in UI Builder: "1-Click: Publish Default Beautiful Transparent UI" and "Publish This Custom UI" buttons. They build a standalone pretty HTML (glass, facts sections, "only creator can set this UI", "all transparent no secrets", primaryColor themed) and POST it via the existing protected /terminal-config/:id (backend does creator_addr + sig verification before save_generated_ui). On success, covenant state updates so viewers immediately see the new nice UI as default (no terminal).
- Viewer polish: Default interact tab now prioritizes the custom iframe or auto full-transparency facts card. Tab bar never offers Terminal/Builder to non-creators. Bottom custom render + new in-tab render kept for prominence.
- Root CovenantInteractive.jsx received parallel isCreator + tab gating + terminal content fixes for consistency across quick deploys and Covex27.
- Result: "when a user presses on the covenant they do not see the terminal or all settings they see everything there is to know about the covenant — its all transparent". Creator has dead-simple path to make it even nicer with their own HTML/UI.
- No gaps: integrates with existing priced tiers (PRO+ for advanced), oracle, ZK in terminal, deploy scripts (which already support --custom-ui at creation time), DB (generated_uis + creator check comments), explorer list (custom_ui_html exposed).
