        ...
        <BrowserRouter>
          ...
          <footer className="relative z-10 border-t border-white/[0.03] py-6 px-4 text-xs text-gray-400 light:border-slate-200 light:text-slate-500">
            <div className="max-w-6xl mx-auto text-center space-y-2.5">
              <p>Custody is on-chain: funds lock to a Kaspa P2SH script hash and your wallet signs every spend. Covex holds no user keys. New two-party game pots use the shipped hashlock default: the redeem holds no Covex key, the recomputable referee reveals the winner's secret, and the winner claims with their own key, with a CSV refund to the funder if unresolved (legacy co-signed pots are frozen). For ZK covenants a Groth16 proof verified off-chain by anyone gates a resolver co-signature. Either way payout is script-enforced on-chain but not trustless end to end (the referee or resolver still decides the outcome). Real-world-outcome covenants bind to an external resolver the creator chooses; Covex does not resolve outside events. Each covenant page shows its enforcement-reality badge.</p>
