...
export default function CopyButton({
  ...
  return (
    <button
      ...
      className={`shrink-0 inline-flex items-center justify-center rounded-md p-1 text-gray-400 light:text-slate-500 hover:text-kaspa-green light:hover:text-emerald-700 hover:bg-white/5 light:hover:bg-slate-100 transition-colors ${className}`}
    >
      {copied ? (
        <Check size={size} className="text-kaspa-green light:text-emerald-700" />
      ) : (
        <Copy size={size} />
      )}
    </button>
