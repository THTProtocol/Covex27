...
// Each entry now includes a `reality` field indicating the verification method:
//   'full-zk'     = Circom/Snarkjs artifacts exist for full on-chain verification
//   'hybrid'      = ZK proves a property (range/membership) + oracle signs the outcome
//   'oracle-attested' = Pure oracle attestation of off-chain execution result
// `artifacts: true` means real .circom + zkey/wasm exist in zk/ directory
export const ZK_CIRCUIT_TYPES = [
  // ═══════════════════════════════════════════
  // GAME CIRCUITS (32 entries) - oracle or hybrid, per-turn timers
  // "only active player clock decrements; <30s red; zero = auto-resolve + submit to oracle"
  // REALITY: All game circuits are oracle-attested for full rules; some add ZK properties
  // ═══════════════════════════════════════════
  { id: 'chess_v1', name: 'Chess (FIDE)', description: 'Full ZK: Groth16 proof per legal move (FIDE rules + timers + draws). MiMC7 FullPositionHasher, witnessed attack verification, castling/en passant/promotion. Falls back to oracle attestation if no proof. Artifacts in zk/games/chess/.', circuit: 'chess_v1', accent: '#49EACB', category: 'game', reality: 'full-zk', artifacts: true },
  { id: 'chess_blitz', name: 'Chess Blitz (3+2)', description: '3 min + 2s increment. FIDE ruleset via oracle. Per-turn timer: only clock of player-to-move ticks. Red <30s. Zero = auto-loss + PAYOUT COMPUTED.', circuit: 'chess_blitz', accent: '#49EACB', category: 'game', variant: true, reality: 'oracle-attested' },
  { id: 'chess_bullet', name: 'Chess Bullet (1+0)', description: '1 min no increment. Oracle-enforced FIDE circuit, aggressive clock. Zero = instant auto-resolve.', circuit: 'chess_bullet', accent: '#49EACB', category: 'game', variant: true, reality: 'oracle-attested' },
  { id: 'poker_v1', name: 'Poker (Texas Hold\'em)', description: 'Off-chain game engine + oracle-attested result. Full hand ranking, pot split, side pots. 2-9 players. Real per-turn timer (blinds/action only current player). Zero = fold + PAYOUT COMPUTED. ZK hand strength + range proof optional.', circuit: 'poker_v1', accent: '#E8AF34', category: 'game', reality: 'hybrid' },
  { id: 'poker_6max', name: 'Poker 6-Max', description: 'Hold\'em 6-max with 15s action timer. Oracle-attested result + optional ZK range proof. Per-turn current player clock only. Red <5s, zero = auto-fold.', circuit: 'poker_6max', accent: '#E8AF34', category: 'game', variant: true, reality: 'hybrid' },
  { id: 'poker_tourney', name: 'Poker Tournament', description: 'Multi-table tournament structure, escalating blinds, ICM chop. Oracle-attested with optional ZK. Per-turn timer per table.', circuit: 'poker_tourney', accent: '#E8AF34', category: 'game', variant: true, reality: 'oracle-attested' },
  { id: 'blackjack_v1', name: 'Blackjack (Full)', description: 'Off-chain engine + oracle-attested result. Hit/stand/double/split/insurance. Dealer reveals after all actions. 6-deck shoe. Per-hand timer - current player only.', circuit: 'blackjack_v1', accent: '#EF4444', category: 'game', reality: 'oracle-attested' },
  { id: 'blackjack_multi', name: 'Blackjack Multi-Hand', description: 'Play up to 3 hands simultaneously. Oracle-attested. Same full rules. Per-hand per-player timer.', circuit: 'blackjack_multi', accent: '#EF4444', category: 'game', variant: true, reality: 'oracle-attested' },
  { id: 'go_9x9', name: 'Go 9×9 Territory', description: 'Japanese rules, territory scoring, komi. Oracle-attested board state. Per-turn timer. Zero = pass + auto-score.', circuit: 'go_9x9', accent: '#22C55E', category: 'game', reality: 'oracle-attested' },
  { id: 'go_13x13', name: 'Go 13×13', description: 'Intermediate board. Oracle-attested rules/timer. Same as 9×9.', circuit: 'go_13x13', accent: '#22C55E', category: 'game', variant: true, reality: 'oracle-attested' },
  { id: 'go_19x19', name: 'Go 19×19 (Full Board)', description: 'Standard full board. Per-turn timer with byo-yomi. Oracle-attested result + optional ZK territory verification.', circuit: 'go_19x19', accent: '#22C55E', category: 'game', variant: true, reality: 'hybrid' },
  { id: 'backgammon_v1', name: 'Backgammon (Doubling Cube)', description: 'Standard rules + doubling cube. Oracle-attested board. VRF for dice fairness. Per-turn timer. Zero = auto-resign.', circuit: 'backgammon_v1', accent: '#F59E0B', category: 'game', reality: 'hybrid' },
  { id: 'connect4_v1', name: 'Connect Four', description: '7×6 board, 4-in-a-row. Oracle-attested gameplay. Per-turn timer. Zero = auto-forfeit.', circuit: 'connect4_v1', accent: '#3B82F6', category: 'game', reality: 'oracle-attested' },
  { id: 'checkers_v1', name: 'Checkers (Standard)', description: '8×8 American checkers, forced jumps, kings. Oracle-attested rules. Per-turn timer. Zero = auto-resolve.', circuit: 'checkers_v1', accent: '#A855F7', category: 'game', reality: 'oracle-attested' },
  { id: 'tictactoe_v1', name: 'Tic-Tac-Toe', description: '3×3 classic. Simple oracle validation. Per-turn timer (short). Zero = draw/forfeit.', circuit: 'tictactoe_v1', accent: '#EC4899', category: 'game', reality: 'oracle-attested' },
  { id: 'reversi_v1', name: 'Reversi / Othello', description: '8×8, capture-flip mechanics. Oracle-attested rules. Per-turn timer.', circuit: 'reversi_v1', accent: '#06B6D4', category: 'game', reality: 'oracle-attested' },
  { id: 'battleship_v1', name: 'Battleship', description: '10×10 fleet placement + salvo. VRF for ship placement fairness. Per-turn (salvo phase). Oracle-attested.', circuit: 'battleship_v1', accent: '#84CC16', category: 'game', reality: 'hybrid' },
  { id: 'scrabble_v1', name: 'Scrabble', description: '15×15 board, dictionary validation, premium squares. Oracle word-check. Per-turn timer.', circuit: 'scrabble_v1', accent: '#F97316', category: 'game', reality: 'oracle-attested' },
  { id: 'dominoes_v1', name: 'Dominoes (Draw)', description: 'Double-6 draw dominoes. Oracle-attested. Per-turn timer. Zero = draw from boneyard + auto-pass.', circuit: 'dominoes_v1', accent: '#14B8A6', category: 'game', reality: 'oracle-attested' },
  { id: 'rummikub_v1', name: 'Rummikub', description: 'Tile sets/runs, 30-point initial meld. Oracle validation of board state. Per-turn timer.', circuit: 'rummikub_v1', accent: '#8B5CF6', category: 'game', reality: 'oracle-attested' },
  { id: 'mancala_v1', name: 'Mancala / Kalah', description: '6-pocket + store per side, capture rules. Oracle-attested. Per-turn timer.', circuit: 'mancala_v1', accent: '#10B981', category: 'game', reality: 'oracle-attested' },
  { id: 'risk_v1', name: 'Risk (Territory)', description: 'Multi-territory conquest, dice combat. VRF dice + oracle. Per-turn (deploy + attack + fortify).', circuit: 'risk_v1', accent: '#DC2626', category: 'game', reality: 'hybrid' },
  { id: 'catan_v1', name: 'Catan (Settlement)', description: 'Resource trading, settlement/road building. VRF dice rolls. Oracle validates trades. Per-turn timer.', circuit: 'catan_v1', accent: '#D946EF', category: 'game', reality: 'hybrid' },
  { id: 'monopoly_v1', name: 'Monopoly (Property)', description: 'Property trading, auctions. VRF dice. Oracle validates trades. Per-turn timer.', circuit: 'monopoly_v1', accent: '#FBBF24', category: 'game', reality: 'hybrid' },
  { id: 'yahtzee_v1', name: 'Yahtzee / Dice', description: '5-dice scorecard game. VRF for dice fairness. Oracle scorecard. Per-turn (3 rolls).', circuit: 'yahtzee_v1', accent: '#34D399', category: 'game', reality: 'hybrid' },
  { id: 'gin_rummy_v1', name: 'Gin Rummy', description: '10-card standard deck, knock/gin/undercut. VRF shuffle. Oracle-attested hand validation. Per-turn timer.', circuit: 'gin_rummy_v1', accent: '#F472B6', category: 'game', reality: 'hybrid' },
  { id: 'hearts_v1', name: 'Hearts (Trick-Taking)', description: '4-player trick-taking, shoot-the-moon. Oracle validates tricks. Per-turn timer.', circuit: 'hearts_v1', accent: '#A78BFA', category: 'game', reality: 'oracle-attested' },
  { id: 'spades_v1', name: 'Spades', description: '4-player partnership, bidding, nil/blind nil. Oracle-attested. Per-turn timer.', circuit: 'spades_v1', accent: '#FB923C', category: 'game', reality: 'oracle-attested' },
  { id: 'bridge_v1', name: 'Bridge (Contract)', description: '4-player, bidding + dummy + play. VRF shuffle + oracle-attested. Per-turn timer.', circuit: 'bridge_v1', accent: '#38BDF8', category: 'game', reality: 'hybrid' },
  { id: 'euchre_v1', name: 'Euchre', description: '4-player trump-based trick game. Oracle-attested. Per-turn timer.', circuit: 'euchre_v1', accent: '#A3E635', category: 'game', reality: 'oracle-attested' },
  { id: 'cribbage_v1', name: 'Cribbage', description: '2-4 player peg-scoring + discard. Oracle counting. Per-turn timer.', circuit: 'cribbage_v1', accent: '#FDA4AF', category: 'game', reality: 'oracle-attested' },
  { id: 'mahjong_v1', name: 'Mahjong (Riichi)', description: '4-player tile-matching with yaku scoring. VRF wall shuffle + oracle-attested. Per-turn timer.', circuit: 'mahjong_v1', accent: '#C084FC', category: 'game', reality: 'hybrid' },

  // ═══════════════════════════════════════════
  // CRYPTO PRIMITIVES (12 entries + 3 new)
  // ═══════════════════════════════════════════
  { id: 'merkle_membership', name: 'Merkle Membership', description: 'Full ZK: proves a key/value pair exists in a committed Merkle tree. Whitelist eligibility, DAO voting power, airdrop claims, token-gated access. Real artifacts in zk/ (circom + snarkjs verifier).', circuit: 'merkle_generic', accent: '#3B82F6', category: 'crypto', reality: 'full-zk', artifacts: true },
  { id: 'merkle_dao', name: 'Merkle DAO Voting', description: 'Full ZK: voting power = merkle leaf value. Threshold quorum. No individual votes revealed. Uses same artifacts as merkle_membership.', circuit: 'merkle_dao', accent: '#3B82F6', category: 'crypto', variant: true, reality: 'full-zk', artifacts: true },
  { id: 'merkle_airdrop', name: 'Merkle Airdrop Claim', description: 'Full ZK: prove eligibility without revealing other claimers. Single-use nullifier per leaf. Same artifacts as membership.', circuit: 'merkle_airdrop', accent: '#3B82F6', category: 'crypto', variant: true, reality: 'full-zk', artifacts: true },
  { id: 'range_proof', name: 'Range Proof', description: 'Full ZK: prove committed value within [min, max] without revealing value. Collateral sufficiency, KYC-free tier qualification. Bulletproofs circuit + snarkjs verifier in zk/ (ceremony in progress for final zkey).', circuit: 'bulletproofs_v1', accent: '#22C55E', category: 'crypto', reality: 'full-zk', artifacts: true },
  { id: 'range_collateral', name: 'Collateral Range Proof', description: 'Full ZK: prove collateral >= loan amount * threshold. Liquidation trigger detection. No amount disclosed. Uses range_proof artifacts.', circuit: 'bulletproofs_collateral', accent: '#22C55E', category: 'crypto', variant: true, reality: 'full-zk', artifacts: true },
  ...
  { id: 'hash_preimage', name: 'Hash Preimage Proof', description: 'Oracle-path (no artifacts yet): prove knowledge of preimage for SHA256/Blake2b hash. Used for HTLC-style covenant script constraints on Kaspa. Simple circuit, planned for artifacts.', circuit: 'hash_preimage', accent: '#F59E0B', category: 'crypto', reality: 'oracle-attested' },
  ...
  // NEW: Merkle-based DAO treasury multisig
  { id: 'dao_multisig', name: 'DAO Treasury Multisig', description: 'Hybrid (oracle-path): M-of-N multisig for DAO treasury. Merkle membership proves voter eligibility. BLS threshold signing for spend authorization. Full ZK artifacts planned for merkle part.', circuit: 'dao_multisig', accent: '#A855F7', category: 'crypto', reality: 'hybrid' },

  // ═══════════════════════════════════════════
  // OWNERSHIP / SCRIPT / TIMELOCK (8 entries + 3 new)
  // ═══════════════════════════════════════════
  { id: 'utxo_ownership', name: 'UTXO Ownership Proof', description: 'Oracle-path (no artifacts yet): prove control of a specific UTXO (address + amount) via Schnorr signature. Used for covenant spend authorization. High priority for artifacts.', circuit: 'utxo_ownership', accent: '#06B6D4', category: 'ownership', reality: 'oracle-attested' },
  { id: 'script_hash_match', name: 'Script Hash Validation', description: 'Oracle-path (no artifacts yet): prove a particular locking script (script_hash) was used. Verify covenant constraints on-chain. SilverScript -> Kaspa Script attestation. Core covenant primitive.', circuit: 'script_hash_match', accent: '#84CC16', category: 'ownership', reality: 'oracle-attested' },
  { id: 'timelock_absolute', name: 'Absolute Timelock Proof', description: 'Oracle-path (no artifacts yet): prove current DAA score >= lock threshold. Vesting schedule, challenge window open verification. Maps to Kaspa DAA consensus primitive.', circuit: 'timelock_abs', accent: '#F97316', category: 'ownership', reality: 'oracle-attested' },
  { id: 'timelock_relative', name: 'Relative Timelock Proof', description: 'Oracle-path (no artifacts yet): prove N blocks passed since reference point. Dispute periods, cooldown windows. High priority for artifacts.', circuit: 'timelock_rel', accent: '#F97316', category: 'ownership', variant: true, reality: 'oracle-attested' },
  { id: 'multisig_threshold', name: 'Multi-Sig Threshold', description: 'Oracle-path (no artifacts yet): prove M-of-N signatures collected. Governance approval, DAO treasury spend authorization. BLS aggregation planned.', circuit: 'multisig_threshold', accent: '#A855F7', category: 'ownership', reality: 'oracle-attested' },
  { id: 'state_transition', name: 'State Transition Proof', description: 'Oracle-path (no artifacts yet): prove valid transition S_i -> S_{i+1} given rules. Off-chain game engine state verification. Used for complex game logic. RISC0/SP1 backend possible.', circuit: 'state_transition', accent: '#10B981', category: 'ownership', reality: 'oracle-attested' },
  { id: 'vesting_schedule', name: 'Vesting Schedule Proof', description: 'Oracle-path (no artifacts yet): prove vested amount at current block height given cliff + linear/sigmoid curve. Token vesting, founder allocation. Uses timelock + range.', circuit: 'vesting_generic', accent: '#34D399', category: 'ownership', reality: 'oracle-attested' },
  { id: 'replay_protection', name: 'Replay Protection', description: 'Oracle-path (no artifacts yet): bound covenant unlock to a specific block hash or DAA window. Anti-replay, chain-specific execution. Maps to Kaspa block hash.', circuit: 'replay_protect', accent: '#F472B6', category: 'ownership', reality: 'oracle-attested' },
  // NEW: Enhanced ownership + script
  { id: 'utxo_spend_auth', name: 'UTXO Spend Authority', description: 'Oracle-path (no artifacts yet): enhanced multi-party UTXO spend. Prove N-of-M UTXO holders authorize a spend. Combines utxo_ownership + multisig_threshold for complex covenant unlocks.', circuit: 'utxo_spend_auth', accent: '#06B6D4', category: 'ownership', variant: true, reality: 'oracle-attested' },
  { id: 'covenant_state_hash', name: 'Covenant State Hash', description: 'Oracle-path (no artifacts yet): prove current covenant state matches committed hash. Used for state channels, optimistic execution verification. Maps to Kaspa covenant state.', circuit: 'state_hash_v1', accent: '#10B981', category: 'ownership', reality: 'oracle-attested' },
  { id: 'script_constraint_proof', name: 'Script Constraint Proof', description: 'Oracle-path (no artifacts yet): prove a specific SilverScript pattern is satisfied (e.g. fee <= X%, creator share = Y). Verifies covenant template parameters match expected values. Critical for SilverScript -> Kaspa attestation.', circuit: 'script_constraint', accent: '#84CC16', category: 'ownership', variant: true, reality: 'oracle-attested' },

  // ═══════════════════════════════════════════
  // DEFI / COLLATERAL (10 entries + 2 new)
  // ═══════════════════════════════════════════
  { id: 'collateral_loan', name: 'Collateralized Loan', description: 'Hybrid: prove locked collateral >= loan * threshold. Liquidation trigger on price feed. ZK range proof for collateral + oracle price attestation. Multi-oracle consensus for price.', circuit: 'collateral_loan', accent: '#DC2626', category: 'defi', reality: 'hybrid' },
  { id: 'yield_accrual', name: 'Yield Accrual Snapshot', description: 'Oracle-attested: verifiable yield/interest calculation over time. A = P * (1+r)^t. Oracle provides rate attestation + on-chain time proof. ZK for calculation accuracy planned.', circuit: 'yield_accrual', accent: '#FBBF24', category: 'defi', reality: 'oracle-attested' },
  { id: 'token_gated', name: 'Token-Gated Access', description: 'Hybrid: prove ownership of a specific token/NFT via merkle + UTXO proof. Gated covenant entry, premium features. Uses merkle_membership artifacts for the ZK part.', circuit: 'token_gated', accent: '#D946EF', category: 'defi', reality: 'hybrid' },
  { id: 'pot_distribution', name: 'Multi-Party Pot Split', description: 'Oracle-attested: verifiable split of total pot among N participants. Weighted by stake, score, or predefined shares. On-chain verifiable math. ZK planned.', circuit: 'pot_split', accent: '#FB923C', category: 'defi', reality: 'oracle-attested' },
  { id: 'escrow_2party', name: '2-Party Escrow', description: 'Oracle-attested: timelocked escrow with mutual release or timeout refund. Oracle dispute resolution option. Uses timelock + range proofs.', circuit: 'escrow_2party', accent: '#38BDF8', category: 'defi', reality: 'oracle-attested' },
  { id: 'escrow_multiparty', name: 'Multi-Party Escrow', description: 'Oracle-attested: N-party escrow with M-of-N release threshold. Milestone-based with oracle verification. Combines timelock + multisig.', circuit: 'escrow_multi', accent: '#38BDF8', category: 'defi', variant: true, reality: 'oracle-attested' },
  { id: 'auction_dutch', name: 'Dutch Auction', description: 'Oracle-attested: price descends linearly over blocks. First bidder wins at current price. Verifiable price curve via oracle timestamp + range.', circuit: 'auction_dutch', accent: '#A78BFA', category: 'defi', reality: 'oracle-attested' },
  { id: 'auction_english', name: 'English Auction', description: 'Oracle-attested: ascending bids, reserve price, time extension on late bids. Verifiable bid ordering. Oracle timestamp.', circuit: 'auction_english', accent: '#A78BFA', category: 'defi', variant: true, reality: 'oracle-attested' },
  { id: 'lending_pool', name: 'Lending Pool Share', description: 'Oracle-attested: prove proportional share of a lending pool. Deposit/withdraw with verifiable exchange rate. On-chain TVL attestation. ZK planned.', circuit: 'lending_pool', accent: '#10B981', category: 'defi', reality: 'oracle-attested' },
  { id: 'prediction_market', name: 'Prediction Market', description: 'Oracle-attested: binary/ternary outcome market. Stake on outcome. Oracle resolves winner. Verifiable payout ratios. Multi-oracle for critical markets.', circuit: 'prediction_market', accent: '#F59E0B', category: 'defi', reality: 'oracle-attested' },
  // NEW: Liquidation + yield extensions
  { id: 'liquidation_threshold', name: 'Liquidation Threshold', description: 'Hybrid: prove collateral value < liquidation threshold via oracle price + ZK range proof. Automatic liquidation trigger for under-collateralized positions. Multi-oracle price consensus.', circuit: 'liq_threshold', accent: '#DC2626', category: 'defi', reality: 'hybrid' },
  { id: 'yield_compounding', name: 'Compounding Yield Proof', description: 'Oracle-attested: verifiable compound interest over N periods. A = P * (1+r)^n with oracle-attested rate per period. Useful for lending pools, staking rewards, DAO treasury growth.', circuit: 'yield_compound', accent: '#FBBF24', category: 'defi', variant: true, reality: 'oracle-attested' },

  // ═══════════════════════════════════════════
  // VERIFIABLE COMPUTE (8 entries + 2 new)
  // ═══════════════════════════════════════════
  { id: 'verifiable', name: 'Verifiable Compute (General)', description: 'Oracle-path (no artifacts yet): arbitrary computation proof via RISC0/SP1 backend. Custom predicates, state transitions, off-chain execution. Requires ceremony + zkey for production use.', circuit: 'risc0_generic', accent: '#A855F7', category: 'compute', reality: 'oracle-attested' },
  { id: 'game_ai_verify', name: 'Game AI Move Validation', description: 'Oracle-path (no artifacts yet): prove a chess/Go AI move is legal + optimal according to engine eval. Prevents collusion. Requires compute proof artifacts.', circuit: 'game_ai_verify', accent: '#A855F7', category: 'compute', variant: true, reality: 'oracle-attested' },
  { id: 'wasm_execution', name: 'WASM Execution Proof', description: 'Oracle-path (no artifacts yet): prove deterministic execution of a WebAssembly module. Custom games, financial formulas, predicates in any language compiled to WASM. Requires WASM ZK backend.', circuit: 'wasm_generic', accent: '#6366F1', category: 'compute', reality: 'oracle-attested' },
  { id: 'ml_inference', name: 'ML Inference Attestation', description: 'Oracle-path (no artifacts yet): prove a machine learning model produced a specific output for given input. Credit scoring, fraud detection, game skill rating. Requires ezkl or similar backend.', circuit: 'ml_inference', accent: '#8B5CF6', category: 'compute', reality: 'oracle-attested' },
  { id: 'financial_formula', name: 'Financial Formula Proof', description: 'Oracle-attested: prove complex financial calculation: Black-Scholes, bond yield, DCF, IRR. Oracle price feed input + verifiable output. ZK compute planned.', circuit: 'financial_formula', accent: '#EC4899', category: 'compute', reality: 'oracle-attested' },
  { id: 'sorting_proof', name: 'Sorting / Ranking Proof', description: 'Oracle-path (no artifacts yet): prove a list is correctly sorted without revealing elements. Tournament rankings, leaderboard verification. Requires sorting circuit artifacts.', circuit: 'sorting_generic', accent: '#06B6D4', category: 'compute', reality: 'oracle-attested' },
  { id: 'graph_reachability', name: 'Graph Reachability', description: 'Oracle-path (no artifacts yet): prove a path exists between two nodes in a committed graph. Supply chain provenance, social graph attestation. Requires graph circuit artifacts.', circuit: 'graph_reach', accent: '#84CC16', category: 'compute', reality: 'oracle-attested' },
  { id: 'zk_email', name: 'ZK Email / DKIM', description: 'Oracle-path (no artifacts yet): prove an email was sent/received per DKIM signature without revealing content. Receipt verification, account recovery. Requires ZK email artifacts.', circuit: 'zk_email', accent: '#F97316', category: 'compute', reality: 'oracle-attested' },
  // NEW: State machine + WASM game rule
  { id: 'basic_state_machine', name: 'Basic State Machine (FSM)', description: 'Oracle-attested: simple finite state machine for turn-based custom games. Define states + valid transitions. Oracle validates each transition. Useful for custom game logic without full ZK.', circuit: 'state_machine', accent: '#10B981', category: 'compute', reality: 'oracle-attested' },
  { id: 'wasm_predicate', name: 'WASM Predicate', description: 'Oracle-path (no artifacts yet): user-defined WASM predicate for game rules or financial logic. Compile any language to WASM, oracle validates execution trace. Custom game logic in Python/Rust/C.', circuit: 'wasm_predicate', accent: '#6366F1', category: 'compute', variant: true, reality: 'oracle-attested' },

  // ═══════════════════════════════════════════
  // GATING / ACCESS (NEW category - 2 entries)
  // ═══════════════════════════════════════════
  { id: 'nft_gating', name: 'NFT Ownership Gating', description: 'Hybrid: prove ownership of a specific NFT collection via merkle + UTXO proof. Gated covenant entry, premium game access, token-gated communities. Uses merkle artifacts.', circuit: 'nft_gating', accent: '#D946EF', category: 'gating', reality: 'hybrid' },
  { id: 'reputation_threshold', name: 'Reputation Threshold', description: 'Oracle-path (no artifacts yet): prove on-chain reputation >= threshold via nullifier + range proof. Reputation-based matchmaking, trust scoring. Nullifier prevents double-counting.', circuit: 'rep_threshold', accent: '#FB923C', category: 'gating', reality: 'oracle-attested' },

  // ═══════════════════════════════════════════
  // OTHER (2 entries - de-prioritized, not primary for p2p covenants)
  // ═══════════════════════════════════════════
  { id: 'age_verification', name: 'Age Verification (KYC-free)', description: 'Oracle-path (no artifacts yet): prove birthdate >= N years before reference. ZK alternative to KYC - no PII revealed. Low priority for p2p covenants. Artifacts planned post high-priority circuits.', circuit: 'age_verify_v1', accent: '#9CA3AF', category: 'other', reality: 'oracle-attested' },
  { id: 'kyc_alternative', name: 'KYC Alternative', description: 'Oracle-path (no artifacts yet): generic credential proof without centralized issuer. Low priority - not primary for Kaspa p2p covenant use-cases. Deferred.', circuit: 'kyc_alt', accent: '#9CA3AF', category: 'other', reality: 'oracle-attested' },

  // ═══════════════════════════════════════════
  // CUSTOM (1 entry)
  // ═══════════════════════════════════════════
  { id: 'custom', name: 'Custom Circuit', description: 'Supply any audited circuit definition and verifier key. Reality depends on your artifacts. Full flexibility for novel covenant types.', circuit: 'custom', accent: '#E8AF34', category: 'custom', reality: 'oracle-attested' },
];
