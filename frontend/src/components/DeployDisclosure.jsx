...
//   'hybrid'          - on-chain custody/payout, but a disclosed oracle gates which
//                       branch releases (markets, oracle covenants) - NOT trustless
//   'oracle-attested' - a disclosed oracle co-signs the outcome - NOT trustless
  ...
  return (
    <div className={`rounded-xl border p-4 ${tone.wrap} ${className}`}>
      ...
      <ul className="space-y-1.5 text-[11px] text-gray-300 light:text-slate-600 leading-relaxed">
        ...
        {isHybrid && <li className="flex gap-2"><span className="text-gray-500">•</span> Custody and payout settle on-chain, but which branch releases is decided by the disclosed Covex oracle (a revealed market outcome, or a proof it verifies). This is NOT trustless.</li>}
        {isOracle && <li className="flex gap-2"><span className="text-gray-500">•</span> The outcome is co-signed by the disclosed Covex oracle (checked off-chain), not by Kaspa consensus alone. This is NOT trustless.</li>}
        {isFullZk && <li className="flex gap-2"><span className="text-gray-500">•</span> A real Groth16 proof is verified off-chain by the disclosed Covex oracle, then co-signed on-chain. Payout still needs that oracle co-signature, so it is NOT trustless end-to-end.</li>}
