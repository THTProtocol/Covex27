        ...
        <div className="relative z-10 p-5 space-y-3">
          ...
          <Section icon={Cpu} title="Where verification happens" accent={ui.accent}>
            ...
            {reality === 'full-zk' && (
              <p>
                ...
                then verified off-chain by you, the counterparty, or any external verifier (snarkjs against the served vkey) before any co-signature is issued. Kaspa has no on-chain pairing verifier, so a valid proof gates a 2-of-2 cosign plus a CSV timeout. An invalid or missing proof is rejected.
              </p>
            )}
          ...
          <Section icon={AlertTriangle} title="What this does not prove" accent="#9ca3af">
            <ul className="list-disc pl-4 space-y-1 text-[12px] text-gray-400 light:text-slate-500">
              {hasZk && <li>Proof verification happens off-chain: anyone (you, the counterparty, any external verifier) can re-run snarkjs against the served vkey. Kaspa has no on-chain pairing verifier on this path, so a valid proof gates a 2-of-2 cosign plus a CSV timeout.</li>}
              {reality === 'on-chain-zk' && <li>Covex keeps this gated until the KIP-16 on-chain ZK settle path is proven on its own prover infrastructure, so treat on-chain ZK settlement as a not-yet-live capability until then, not a guarantee.</li>}
