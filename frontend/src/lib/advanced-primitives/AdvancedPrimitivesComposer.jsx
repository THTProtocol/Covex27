...
export default function AdvancedPrimitivesComposer({ initialConfig, onChange }) {
  ...
  return (
    <Card className="p-6 space-y-6" accent="#49EACB">
      <div>
        <h3 className="text-lg font-bold text-white light:text-slate-900 mb-1">Advanced Primitives Composer</h3>
        <p className="text-xs text-gray-400 light:text-slate-500">Build complex logic visually. This config will be compiled into your SilverScript + resolution rules.</p>
      </div>

      {/* Time Locks */}
      <div className="border border-white/10 light:border-slate-200 rounded-2xl p-4">
        <div className="flex items-center gap-2 mb-3">
          <span className="text-[#49EACB]"><Clock size={18} /></span>
          <span className="font-semibold text-white light:text-slate-900">Time Locks</span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
          <div>
            <label className="text-xs text-gray-400 light:text-slate-500">Release After (ISO)</label>
            <input
              type="datetime-local"
          ...
          <div>
            <label className="text-xs text-gray-400 light:text-slate-500">Challenge Period (hours)</label>
            <input
              type="number"
              className="cyber-input w-full px-3 py-2 text-sm"
              placeholder="72"
      ...
      <div className="border border-white/10 light:border-slate-200 rounded-2xl p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="font-semibold text-white light:text-slate-900">Multi-Party Approval</span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
          <div>
            <label className="text-xs text-gray-400 light:text-slate-500">Required Approvals (M-of-N)</label>
            <input
              type="number"
              className="cyber-input w-full px-3 py-2 text-sm"
              placeholder="2"
          ...
          <div>
            <label className="text-xs text-gray-400 light:text-slate-500">Approvers (comma separated addresses)</label>
            <input
              type="text"
              className="cyber-input w-full px-3 py-2 text-sm"
      ...
      <div className="border border-white/10 light:border-slate-200 rounded-2xl p-4">
        <div className="flex justify-between items-center mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-white light:text-slate-900">Cross Conditions</span>
          </div>
          <button onClick={addCondition} className="text-xs px-3 py-1 bg-white/5 hover:bg-white/10 light:bg-slate-100 light:hover:bg-slate-200 light:text-slate-900 rounded">+ Add Condition</button>
        </div>

        {(primitives.conditions || []).length === 0 && (
          <p className="text-xs text-gray-500 light:text-slate-500">No conditions yet. Add ZK + Oracle + Time logic.</p>
        )}
        ...
        {(primitives.conditions || []).map((cond, index) => (
          <div key={index} className="flex gap-2 mb-2 items-center">
            <select
              ...
              className="cyber-input px-2 py-1 text-sm"
            >
            ...
            <input
              ...
              className="cyber-input flex-1 px-2 py-1 text-sm"
      ...
      <div className="border border-white/10 light:border-slate-200 rounded-2xl p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="font-semibold text-white light:text-slate-900">Dispute / Challenge System</span>
          <label className="ml-auto flex items-center gap-2 text-xs light:text-slate-700">
            <input
              type="checkbox"
        ...
        {primitives.dispute?.enabled && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
            <div>
              <label className="text-xs text-gray-400 light:text-slate-500">Bond Amount (KAS)</label>
              <input type="number" className="cyber-input w-full px-3 py-2 text-sm" placeholder="100" />
            </div>
            <div>
              <label className="text-xs text-gray-400 light:text-slate-500">Dispute Window (hours)</label>
              <input type="number" className="cyber-input w-full px-3 py-2 text-sm" placeholder="168" />
            </div>
          </div>
        )}
      </div>

      <div className="text-[10px] text-gray-500 light:text-slate-500 pt-2 border-t border-white/10 light:border-slate-200">
