...
export const STARTER_TEMPLATES = [
  ...
  },
  {
    ...
    data: {
      ...
      content: [
        ...
        blk('Footer', { text: '{{name}} · Game covenant on Kaspa, settled from a replayable move log', showNetwork: 'yes' }),
      ],
    },
  },
  {
  ...
  },
  {
    ...
    name: 'Access Pass',
    premium: true,
    match: ['access', 'pass', 'membership', 'unlock', 'ticket'],
    desc: 'One-time access tiers, a benefits grid, and an on-chain enforcement badge.',
    data: {
      root: ROOT({ backgroundPreset: 'kaspa-hero' }),
      content: [
        blk('HeroImage', { backgroundImageUrl: '', overlayOpacity: '0.55', title: '{{name}}', subtitle: 'A one-time on-chain pass. Pay once in your own wallet, non-custodial and verifiable. {{total_locked}} committed on {{network}}.', ctaLabel: 'Get your pass', ctaAction: 'spend', accentColor: '#49EACB', alignment: 'center' }),
        blk('EnforcementBadge', { note: 'How this pass is enforced' }),
        blk('PricingTier', { columns: '3', tiers: [{ name: 'Supporter', price: '10', cadence: 'KAS one-time', featured: 'no', glow: 'builder', perks: 'Full access\nNon-custodial\nYours forever', ctaLabel: 'Get pass', ctaAction: 'spend', amount: '10' }, { name: 'Member', price: '50', cadence: 'KAS one-time', featured: 'yes', glow: 'pro', perks: 'Full access\nPriority perks\nYours forever', ctaLabel: 'Get pass', ctaAction: 'spend', amount: '50' }, { name: 'Founder', price: '200', cadence: 'KAS one-time', featured: 'no', glow: 'max', perks: 'Full access\nAll future perks\nFounder badge', ctaLabel: 'Get pass', ctaAction: 'spend', amount: '200' }] }),
        blk('FeatureGrid', { columns: '3', features: [{ icon: 'Lock', headline: 'One-time, on-chain', description: 'Pay once. The pass settles on Kaspa and is disclosed and verifiable on the explorer. No recurring charges, ever.' }, { icon: 'ShieldCheck', headline: 'Non-custodial', description: 'You sign the payment in your own wallet. Keys never leave your device.' }, { icon: 'Zap', headline: 'Kaspa speed', description: 'Settles on the BlockDAG at 10 blocks per second.' }] }),
        blk('StatRow', { stats: [{ label: 'Holders', value: '{{tx_count}}' }, { label: 'Committed', value: '{{total_locked}}' }, { label: 'Status', value: '{{status}}' }] }),
        blk('Accordion', { title: 'Frequently asked', items: [{ q: 'Is this a subscription?', a: 'No. It is a single one-time payment for access. Nothing recurs and nothing auto-charges. See the enforcement badge on this page.' }, { q: 'Is it non-custodial?', a: 'Yes. You sign in your own wallet; keys never leave your device.' }] }),
        blk('Footer', { text: '{{name}} · One-time access pass on Kaspa', showNetwork: 'yes' }),
      ],
    },
  },
  {
    ...
    name: 'DAO / Governance',
    premium: true,
    match: ['dao', 'governance', 'proposal', 'vote', 'treasury'],
    desc: 'Proposal hero, a live for/against tally, a lifecycle timeline, and a vote CTA.',
    data: {
      root: ROOT({ backgroundPreset: 'midnight' }),
      content: [
        blk('HeroImage', { backgroundImageUrl: '', overlayOpacity: '0.55', title: '{{name}}', subtitle: 'On-chain governance on Kaspa. {{total_locked}} in the treasury on {{network}}.', ctaLabel: 'Cast your vote', ctaAction: 'interact', accentColor: '#49EACB', alignment: 'center' }),
        blk('EnforcementBadge', { note: 'How this proposal is enforced' }),
        blk('OddsBar', { labelA: 'FOR', valueA: '{{pool_yes}}', colorA: '#49EACB', labelB: 'AGAINST', valueB: '{{pool_no}}', colorB: '#F472B6', showOdds: 'no' }),
        blk('StatRow', { stats: [{ label: 'Treasury', value: '{{total_locked}}' }, { label: 'Votes', value: '{{tx_count}}' }, { label: 'Status', value: '{{status}}' }] }),
        blk('Timeline', { title: 'Proposal lifecycle', steps: [{ label: 'Proposed', detail: 'A signer opens the proposal on-chain.', state: 'done' }, { label: 'Voting', detail: 'Members vote; the tally is live and public.', state: 'active' }, { label: 'Execution', detail: 'On quorum the timelocked action executes; otherwise it refunds.', state: 'upcoming' }] }),
        blk('Leaderboard', { title: 'Top voters', rankBy: 'amount', maxRows: '5', emptyText: 'No votes on-chain yet. Be the first to weigh in.', accentColor: '#E8AF34' }),
        blk('Accordion', { title: 'Frequently asked', items: [{ q: 'How is the vote enforced?', a: 'The tally and any treasury movement are on-chain and verifiable on the explorer. See the enforcement badge on this page (script-enforced, oracle-cosigned, or full-zk verified off-chain).' }, { q: 'Is it non-custodial?', a: 'Yes. You sign your vote in your own wallet; keys never leave your device.' }] }),
        blk('Footer', { text: '{{name}} · Governance covenant on Kaspa', showNetwork: 'yes' }),
      ],
    },
  },
  {
    id: 'nft-drop',
    name: 'Collectible Drop',
    premium: true,
    match: ['nft', 'drop', 'collectible', 'mint', 'edition'],
    desc: 'Mint hero, a supply meter, rarity tiers, and a live mint feed.',
    data: {
      root: ROOT({ backgroundPreset: 'gold-prestige', accentColor: '#E8AF34' }),
      content: [
        blk('HeroImage', { backgroundImageUrl: '', overlayOpacity: '0.55', title: '{{name}}', subtitle: 'Mint in your own wallet. Non-custodial, on-chain, verifiable. {{total_locked}} minted on {{network}}.', ctaLabel: 'Mint now', ctaAction: 'spend', accentColor: '#E8AF34', alignment: 'center' }),
        blk('EnforcementBadge', { note: 'How this drop is enforced' }),
        blk('PoolMeter', { label: 'Minted so far', value: '{{amount_kaspa}}', max: '10000', suffix: 'KAS', color: '#E8AF34' }),
        blk('PricingTier', { columns: '3', tiers: [{ name: 'Common', price: '5', cadence: 'KAS mint', featured: 'no', glow: 'builder', perks: 'Open edition\nNon-custodial mint', ctaLabel: 'Mint', ctaAction: 'spend', amount: '5' }, { name: 'Rare', price: '25', cadence: 'KAS mint', featured: 'yes', glow: 'pro', perks: 'Limited supply\nNon-custodial mint\nRarity badge', ctaLabel: 'Mint', ctaAction: 'spend', amount: '25' }, { name: '1-of-1', price: '250', cadence: 'KAS mint', featured: 'no', glow: 'max', perks: 'Unique piece\nNon-custodial mint\nFounder status', ctaLabel: 'Mint', ctaAction: 'spend', amount: '250' }] }),
        blk('ActivityFeed', { title: 'Recent mints', emptyText: 'No mints on-chain yet. Be the first to collect.', maxRows: '5', accentColor: '#E8AF34' }),
        blk('Accordion', { title: 'Frequently asked', items: [{ q: 'What am I minting?', a: 'An on-chain Kaspa covenant edition. The mint and ownership are disclosed and verifiable on the explorer. See the enforcement badge on this page.' }, { q: 'Is it non-custodial?', a: 'Yes. You sign the mint in your own wallet; keys never leave your device.' }] }),
        blk('Footer', { text: '{{name}} · Collectible drop on Kaspa', showNetwork: 'yes' }),
      ],
    },
  },
  {
