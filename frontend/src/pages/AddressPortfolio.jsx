    ...
    <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 py-10">
      ...
      <div className="flex items-start gap-4 mb-8">
        ...
        <div className="min-w-0">
          <h1 className="text-2xl font-black text-white mb-1">Address Portfolio</h1>
          <button onClick={copy} className="flex items-center gap-2 text-xs font-mono text-gray-400 hover:text-kaspa-green transition-colors break-all text-left">
            {addr} {copied ? <Check size={12} className="text-kaspa-green shrink-0" /> : <Copy size={12} className="shrink-0" />}
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="glass-panel rounded-2xl py-16 text-center">
          <p className="text-gray-300 font-semibold mb-1">No covenants created by this address on {network}</p>
          <p className="text-gray-500 text-xs">Covenants appear here as soon as the indexer discovers them on-chain.</p>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-3 gap-4 mb-8">
            {[
              ...
              <div key={i} className="glass-panel rounded-2xl p-4 flex items-center gap-3">
                ...
                <div>
                  <p className="text-[10px] text-gray-400 font-mono uppercase tracking-wider">{s.label}</p>
            ...
            {data.covenants.map((c) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <span className="font-bold text-white truncate">{c.name || c.covenant_type || 'Covenant'}</span>
                  <span className="text-xs font-mono text-kaspa-green shrink-0">{c.amount_kaspa} KAS</span>
                </div>
                <p className="text-xs text-gray-400 line-clamp-2 mb-3">{c.description || c.full_logic_summary || 'On-chain Kaspa covenant.'}</p>
                <div className="flex items-center justify-between text-[10px] font-mono text-gray-500">
                  <span>{c.category}</span>
                  <span className={c.verified_tier !== 'FREE' ? 'text-amber-300' : ''}>{c.verified_tier}</span>
                </div>
              </Link>
            ))}
          </div>
