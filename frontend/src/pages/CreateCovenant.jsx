...
/* ───────────────────────────────────────────────────────────────────
   Create Covenant — SilverScript editor with compiler integration
   ─────────────────────────────────────────────────────────────────── */

// Deployment address template — replace with real deployer address
const DEPLOYER_ADDRESS = 'kaspatest:qzr8q7tq8w3n2x3a4y5z6w7x8c9d0eqqqqqqqqqqqqqqqqqqqqqqqqqq';
...
export default function CreateCovenant() {
  ...
  // Build deployment wallet URI when we have a script hash
  const deployUri = useMemo(() => {
    if (!result?.script_template_hash) return null;
    // Encode the script hash and optional amount for deployment
    const hash = result.script_template_hash;
    return `kaspatest:${DEPLOYER_ADDRESS}?amount=1&scriptHash=${hash}`;
  }, [result]);

  const canCompile = code.trim().length > 0 && !loading;
  ...
  return (
    <div className="w-full max-w-3xl mx-auto px-4 py-10 space-y-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Write SilverScript (<code className="text-kaspa-gold text-xs bg-black/30 px-1.5 py-0.5 rounded font-mono">.sil</code>) code and deploy directly to the Kaspa network
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="glass p-8 space-y-6">
        <div>
          ...
          <textarea
            ...
            placeholder={`// Write your SilverScript covenant here…\n// Example:\n\ncovenant TransferWithTimeout {\n    payer: Address;\n    payee: Address;\n    amount: u64;\n    timeout: DaaScore;\n\n    predicate transfer() {\n        require(sig(payer));\n        require(op() == Op::Transfer);\n        lock amount to payee;\n    }\n\n    predicate reclaim() {\n        require(sig(payer));\n        require(daaScore() >= timeout);\n        unlock amount;\n    }\n}`}
            spellCheck={false}
            className="
              w-full h-72 px-5 py-4 rounded-xl
              bg-black/40 border border-white/10
              text-white font-mono text-sm leading-relaxed
              placeholder:text-gray-700
              focus:outline-none focus:border-kaspa-gold/50 focus:ring-1 focus:ring-kaspa-gold/20
        ...
        <button
          ...
          disabled={!canCompile}
          className={`
            w-full inline-flex items-center justify-center gap-3 px-6 py-3.5 rounded-xl font-semibold text-sm
            transition-all duration-200
            ${canCompile
              ? 'bg-kaspa-gold text-black shadow-[0_0_30px_rgba(232,175,52,0.25)] hover:shadow-[0_0_50px_rgba(232,175,52,0.40)] hover:brightness-110 active:scale-[0.97]'
        ...
        <div className="glass p-6 border-red-500/30">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-red-300/80 mt-1 font-mono leading-relaxed whitespace-pre-wrap">
                {error}
              </p>
            </div>
          </div>
          ...
          {result.bytecode && (
            <div>
              <p className="text-xs text-gray-500 mb-1.5">
                Bytecode <span className="text-gray-600">({result.bytecode.length} hex chars)</span>
              </p>
              <div className="p-4 rounded-xl bg-black/40 border border-white/5 font-mono text-xs text-gray-400 break-all max-h-32 overflow-y-auto leading-relaxed">
                {result.bytecode}
              </div>
            </div>
          )}
      ...
      <div className="glass p-5 text-xs text-gray-600 leading-relaxed">
        <p>
          SilverScript code is compiled locally via the Rust backend using the
          native <code className="text-gray-500">silverc</code> compiler.
          Covenants are deployed directly to the Kaspa network. Covex does not
          audit your code — review it carefully before deployment.
        </p>
      </div>
    </div>
  );
