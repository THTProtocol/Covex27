...
// co-signature gating a 2-of-2 cosign plus a CSV timeout. The KIP-16
// OpZkPrecompile is a separate on-chain ZK path the settlement covenant targets,
// gated until proven live on Kaspa.
...
const SECTIONS = [
  {
    ...
    body: [
      "Kaspa is a covenant-capable Layer 1: a 10 BPS proof-of-work BlockDAG that runs native, stateful, multi-transaction programs over UTXOs. What is missing is the human layer. There is no single place to see every covenant on chain, act on any of them from your own wallet, and create new ones without writing raw script. Covex is that layer.",
      "Covex is non-custodial. Keys stay in your wallet, the platform holds no funds, and it signs nothing on your behalf. The circom proof suite Covex ships is verified off chain by anyone, and the only thing the chain checks at unlock is a deployer-bound external resolver's Schnorr co-signature. KIP-16's verification opcode adds a separate on-chain proof path that stays gated until it is proven live on Kaspa. This paper sets out the problem, the design, the trust model, and exactly what is guaranteed today.",
    ],
  },
  {
    id: 'background', n: '1 · Background: covenants on Kaspa',
    body: [
      "Kaspa is a proof-of-work BlockDAG ordered by the GHOSTDAG and DAGKNIGHT protocols. Since the Crescendo hard fork (around May 2025) it produces 10 blocks per second while preserving Nakamoto-style security, with a roadmap toward 100 BPS. Crescendo also shipped the KIP-10 transaction-introspection opcodes, the first step toward covenants.",
      "Kaspa completes the covenant story with four improvement proposals now live: KIP-17, the extended script-engine opcodes that form the covenant backbone; KIP-20, covenant IDs that give stable identity and lineage; KIP-16, a zero-knowledge opcode set whose OpZkPrecompile verifies a RISC0-Groth16 proof in consensus; and KIP-21, partitioned sequencing commitments.",
      "For real-value settlement today, Covex does not yet rely on that on-chain proof opcode. Covex keeps the KIP-16 settlement path gated until it is proven live on Kaspa. Until then, the guarantee Covex offers is on-chain Schnorr verification of a deployer-bound resolver's co-signature, with the circom proof verified off chain. The trust model section sets this out in full.",
      "SilverScript, a CashScript-inspired language and compiler, compiles covenants down to Kaspa script. Covex builds directly on this stack, and covenants settle with real funds on Kaspa today.",
    ],
  },
  {
    id: 'problem', n: '2 · Problem',
    takeaway: "Now that covenants are live on Kaspa, three gaps open at once: finding them on the DAG, acting on them from a wallet, and authoring them without locking funds in raw script.",
    body: [
      "A programmable UTXO is invisible without infrastructure. With covenants live on Kaspa, three gaps appear together.",
  ...
  },
  {
    ...
    body: [
      ...
      "Two-party game outcomes are the one place a Covex key appears, and that path is simulated, not chain-enforced. Covex re-derives the result from the publicly replayable signed move log, which anyone can recompute, and co-signs the payout; it does not decide the result, and the chain still requires the winning party's own signature. Covex is migrating these to an external-referee model to remove even that key. Markets, ZK proofs, and real-value covenants on Kaspa never use a Covex key.",
      "Two things are on the horizon, not in force. The chain-enforced, no-Covex-key path (the same external-resolver hashlock the conditional-outcome covenants use) is rolling out, and KIP-16's OpZkPrecompile, which verifies a RISC0-Groth16 proof in consensus, is the on-chain path the settlement covenant targets once it is proven live on Kaspa. The trusted setup behind today's circom proofs is a single-contributor development ceremony, not a production multi-party MPC. This is the trust assumption today, disclosed rather than hidden.",
  ...
  },
  {
    ...
    body: [
      `Today, all ${ZK_TOTAL_WORD} compiled circom circuits in the registry are verified off chain, and the chain enforces only a deployer-bound external resolver's Schnorr co-signature at unlock, exactly as the trust model describes. No circom proof is enforced end to end on Kaspa, because the covenant builder contains no circuit-output to hashlock binding. The circom set is verified off chain.`,
      "KIP-16's OpZkPrecompile does verify a RISC0-Groth16 proof in consensus, and the settlement covenant migrates onto that on-chain path as its proving keys and a real multi-party ceremony ship. It stays gated until it is proven live on Kaspa.",
  ...
  },
  {
    ...
    body: [
      "The platform that indexes covenants best at the moment they appear becomes the default explorer for the category. Covex has proven its indexer at scale and runs a live Kaspa node indexing covenants as they land.",
      "The goal of this codebase is simple: to be correct, honest, and complete for covenants on Kaspa.",
    ],
  },
];
    ...
    <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 py-10">
      ...
      <Card hero className="relative z-10 p-5 mb-8 flex items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="text-sm text-gray-400 light:text-slate-500 break-words">A Covenant Explorer and Studio for Kaspa · v1.4 · 2026-06-30</p>
        </div>
      </Card>
      ...
      <Card className="mt-12 p-5">
        ...
        <div className="mt-2 flex flex-col gap-1.5 text-xs">
          {[
            ['Kaspa covenants design overview (Michael Sutton)', 'https://medium.com/@michaelsuttonil/kaspa-covenants-toccata-hard-fork-outlook-a4d81a40900c'],
