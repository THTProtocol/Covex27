...
const SECTIONS = [
  ...
  },
  {
    ...
    body: [
      `Today, all ${ZK_TOTAL_WORD} compiled circom circuits in the registry are verified off-chain by you, the counterparty, or any external resolver. Four of them (merkle_membership, age_verification, escrow_2party, range_proof) reduce to pure on-chain primitives and are trustless end-to-end; for the rest (timelock, split-math, VRF and so on) only a deployer-bound external resolver's Schnorr co-signature is checked on-chain at unlock, so trust sits with that disclosed resolver, not Covex. Beyond the four self-contained circuits, no circom circuit's ZK proof is enforced end-to-end on Kaspa: there is no circuit-output to hashlock binding in the covenant builder, so the chain never checks those proofs. Toccata's KIP-16 OpZkPrecompile does verify a RISC0-Groth16 proof in consensus, and the settlement covenant migrates onto that on-chain path as its proving keys and a real multi-party ceremony ship; it stays testnet-gated until proven live on mainnet. At the 30 June 2026 mainnet launch the circom set is verified off-chain.`,
