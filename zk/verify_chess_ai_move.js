...
/** Hybrid for chess_ai_move. Real when possible. */
const VKEY_PATH = path.join(__dirname, "chess_ai_move_vkey.json");

async function main() {
  const proofFile = process.argv[2];
  const circuit = process.argv[3] || "chess_ai_move";
  ...
  if (fs.existsSync(VKEY_PATH) && hasFull) {
    try {
      ...
      if (valid) {
        console.log(JSON.stringify({valid:true, publicSignals, circuit, note:"real groth16 chess ai"}));
        process.exit(0);
      }
    } catch(_) {}
  }
