...
/** Hybrid for loan_health. Real groth when vkey+body present, attested fallback. */
const VKEY_PATH = path.join(__dirname, "loan_health_vkey.json");

async function main() {
  const proofFile = process.argv[2];
  const circuit = process.argv[3] || "loan_health";
  ...
  if (fs.existsSync(VKEY_PATH) && hasFull) {
    try {
      ...
      if (valid) {
        console.log(JSON.stringify({valid:true, publicSignals, circuit, note:"real groth16 loan health"}));
        process.exit(0);
      }
    } catch(_) {}
  }
