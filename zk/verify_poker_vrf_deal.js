...
/** Hybrid for poker_vrf_deal (VRF deck deal fairness). Real groth when possible, attested fallback for oracle. */
const VKEY_PATH = path.join(__dirname, "poker_vrf_deal_vkey.json");

async function main() {
  const proofFile = process.argv[2];
  const circuit = process.argv[3] || "poker_vrf_deal";
  ...
  if (fs.existsSync(VKEY_PATH) && hasFull) {
    try {
      ...
      if (valid) {
        console.log(JSON.stringify({valid: true, publicSignals, circuit, note: "real groth16 vrf deal"}));
        process.exit(0);
      }
      // fallthrough on fail for hybrid attested behavior
    } catch(_) {}
  }
