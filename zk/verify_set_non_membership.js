...
/**
 * verify_set_non_membership.js — STRICT Groth16 verifier for set_non_membership (Covex27).
 ...
 */
const VKEY_PATH = path.join(__dirname, "../frontend/public/zk/set_non_membership/set_non_membership_vkey.json");

async function main() {
    const proofFile = process.argv[2];
    if (!proofFile) {
        console.log(JSON.stringify({ valid: false, error: "Usage: node verify_set_non_membership.js <proof.json>" }));
        process.exit(1);
    }
    ...
    try {
        ...
        console.log(JSON.stringify({ valid, publicSignals, circuit: "set_non_membership" }));
    } catch (e) {
