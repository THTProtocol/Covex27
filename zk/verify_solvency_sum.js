...
/**
 * verify_solvency_sum.js — STRICT Groth16 verifier for solvency_sum (proof of reserves) (Covex27).
 ...
 */
const VKEY_PATH = path.join(__dirname, "../frontend/public/zk/solvency_sum/solvency_sum_vkey.json");

async function main() {
    const proofFile = process.argv[2];
    if (!proofFile) {
        console.log(JSON.stringify({ valid: false, error: "Usage: node verify_solvency_sum.js <proof.json>" }));
        process.exit(1);
    }
    ...
    try {
        ...
        console.log(JSON.stringify({ valid, publicSignals, circuit: "solvency_sum" }));
    } catch (e) {
