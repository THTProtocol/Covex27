...
// SERVED frontend artifact (frontend/public/zk/tictactoe_v1/) is the single source of
...
const VKEY_CANDIDATES = [
    path.join(__dirname, "..", "frontend", "public", "zk", "tictactoe_v1", "tictactoe_v1_vkey.json"),
    path.join(__dirname, "tictactoe_v1_vkey.json"),
    path.join(__dirname, "games/tictactoe/output/tictactoe_v1_vkey.json"),
];
...
async function main() {
    ...
    if (!proofFile) {
        console.log(JSON.stringify({ valid: false, error: "Usage: node verify_tictactoe.js <proof.json>" }));
        process.exit(1);
    }
    ...
    try {
        ...
        console.log(JSON.stringify({ valid, publicSignals, circuit: "tictactoe_v1" }));
    } catch (e) {
