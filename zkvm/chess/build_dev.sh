#!/usr/bin/env bash
# Dev-mode build + run: compiles the guest (shakmaty -> riscv32im-risc0-zkvm-elf) and host, then
# runs in RISC0_DEV_MODE=1 (fast, no real STARK) to flush out COMPILE issues before the slow
# real proof. NOTE: dev mode is NOT a real proof; it is only to verify the toolchain builds.
set -uo pipefail
